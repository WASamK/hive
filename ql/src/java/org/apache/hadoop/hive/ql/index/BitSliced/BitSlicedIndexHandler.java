package org.apache.hadoop.hive.ql.index.BitSliced;
...
/**
 * Created by samadhik on 1/11/17.
 */
public class BitSlicedIndexHandler extends TableBasedIndexHandler {
    ...
    public void generateIndexQuery(List<Index> indexes, ExprNodeDesc predicate,
        ...
        qlCommand.append("SELECT bucketname AS `_bucketname` , EWAH_BITMAP_POSITIONS(bitmaps) AS `_offsets` FROM ");
        qlCommand.append("(SELECT `_bucketname` AS bucketname ,");
        qlCommand.append(" `_bitmaps` AS bitmaps FROM ");
        ...
        qlCommand.append(" WHERE NOT EWAH_BITMAP_EMPTY(" + head.getAlias() + ".`_bitmaps`) ) tmp_index GROUP BY bucketname,bitmaps");
    ...
    public void analyzeIndexDefinition(Table baseTable, Index index,
        ...
        if (this.usesIndexTable() && indexTable != null) {
            ...
            indexTblCols.clear();
            FieldSchema bitmaps = new FieldSchema("_bitmaps", "array<array<bigint>>", "");
    ...
    protected Task<?> getIndexBuilderMapRedTask(Set<ReadEntity> inputs, Set<WriteEntity> outputs,
        ...
        command.append("ewah_bit_slices(");
        command.append("COLLECT_LIST(");
        command.append("a.");
        ...
        for (FieldSchema fieldSchema : indexField) {
            //command.append(",");//Put an exception if there are more than one column
